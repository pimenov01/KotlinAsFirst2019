...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (i in substrings.indices) {
        val lowTextToChange = lowText.windowed(substrings[i].length)
        for (j in lowTextToChange.indices) {
            if (substrings[i].toLowerCase() == lowTextToChange[j])
                count++
        }
        map += substrings[i] to count
        count = 0
    }
    return map
}
...
fun sibilants(inputName: String, outputName: String) {
    val reader = File(inputName).readText()
        .replace(Regex("""(?<=[ШшЩщЧчЖж])Ы"""), "И")
        .replace(Regex("""(?<=[ШшЩщЧчЖж])ы"""), "и")
        .replace(Regex("""(?<=[ШшЩщЧчЖж])Я"""), "А")
        .replace(Regex("""(?<=[ШшЩщЧчЖж])я"""), "а")
        .replace(Regex("""(?<=[ШшЩщЧчЖж])Ю"""), "У")
        .replace(Regex("""(?<=[ШшЩщЧчЖж])ю"""), "у")
...
fun centerFile(inputName: String, outputName: String) {
    ...
    for (i in reader.indices) {
        if (reader[i].length > max)
            max = reader[i].length
    }

    File(outputName).bufferedWriter().use {
        for (i in reader.indices) {
            val currentLength = reader[i].length
            it.write(" ".repeat((max - currentLength) / 2) + reader[i])
            it.newLine()
        }
    }
}
...
fun top20Words(inputName: String): Map<String, Int> {
    ...
    return map.toList().sortedWith(compareByDescending { (_, count) -> count }).take(20).toMap()
}
 ...
 *
 */
fun printDivisionProcess(lhv: Int, rhv: Int, outputName: String) { //в разработке
    TODO()
}
